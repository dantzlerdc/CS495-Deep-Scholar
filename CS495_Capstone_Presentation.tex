...
\usepackage{graphicx}
\graphicspath{{./}{slides/figures/}{previews/}{assets/figures/}}
\usepackage{booktabs}
\usepackage{xcolor}
\usepackage{fontenc}
\usepackage{lmodern}
...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture,overlay]
    \node[anchor=south west,inner sep=0pt] at (current page.south west)
      {\includegraphics[width=\paperwidth,height=\paperheight]{Slide-Cover.png}};
  \end{tikzpicture}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.58\textwidth}
      ...
      \adjustbox{max width=\linewidth,max height=5.8cm,keepaspectratio}{%
        \includegraphics{Slide-Option-Contract.png}}
    \end{column}
...
\begin{frame}{Forward Pass --- Building the Stock Price Lattice}
  ...
  \begin{center}
    \adjustbox{max width=0.92\textwidth,max height=5.3cm,keepaspectratio}{%
      \includegraphics{Slide-Forward-Pass.png}}
  \end{center}
...
\begin{frame}{Backward Induction --- Where the Option Price Emerges}
  ...
  \begin{center}
    \adjustbox{max width=0.92\textwidth,max height=5.3cm,keepaspectratio}{%
      \includegraphics{Slide-Backward-Pass.png}}
  \end{center}
    ...
    \begin{column}{0.54\textwidth}
      ...
      \begin{tikzpicture}[
        ...
        \node[nxtstep] (s1) at (0,0)
          {Expand option contracts to use other stock datasets
           to validate the model against.};
        \node[num,left=0.20cm of s1.west] {1};

        \node[nxtstep,below=0.20cm of s1] (s2)
          {Run the cross-sectional test across multiple expiration
           cycles and multiple high-liquidity stocks
           (e.g., NVDA, MSFT) to prove the pipeline's stability.};
        \node[num,left=0.20cm of s2.west] {2};

        \node[nxtstep,below=0.20cm of s2] (s3)
          {Modernize the Layer 2 ML Model Architecture by replacing
           XGBoost with TabPFN v2 --- a transformer-based foundation
           model for tabular data that delivers state-of-the-art
           calibrated probability estimates without per-task gradient
           boosting training.};
...
\begin{frame}{Live Demo --- Interactive CRR Binomial Pricing Calculator}
  ...
  \begin{columns}[T]
    \begin{column}{0.58\textwidth}
      \centering
      \adjustbox{max width=\linewidth,max height=5.8cm,keepaspectratio}{%
        \includegraphics{Slide-Calculator-Hero.png}}
    \end{column}
