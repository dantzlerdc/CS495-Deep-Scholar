...
\graphicspath{{./}{slides/figures/}{previews/}{project/outputs/}}
...
single contract through both pricing layers, evaluation metrics,
tools, and limitations. Section~4 presents the full results,
organized to mirror the three research hypotheses introduced in
...
and limitations, and identifying directions for future work.
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.85\textwidth]{Slide-Forward-Pass.png}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.85\textwidth]{Slide-Backward-Pass.png}
...
Five limitations bound the scope of the conclusions reported above.

\textbf{Single-underlying empirical sample.} The cross-validation
result is established on AAPL options data spanning 2016--2021. AAPL
is a large-cap, optionable underlying with high liquidity and a
well-developed implied-volatility surface. The herding-regime
correlation of $r = 0.413$ may not generalize to small-cap names with
thinner option markets, to commodity-linked equities with idiosyncratic
volatility regimes, or to indices where the realized-vs-implied
volatility spread behaves structurally differently. Cross-asset
validation is identified as future work below.

\textbf{No-dividend CRR engine.} The Layer-1 lattice is implemented
without dividend adjustment, which introduces a systematic downward
bias on put model prices for dividend-paying underlyings (the AAPL
sample's modest dividend yield notwithstanding). This explains the
Q1-dominant agreement zone in
Figure~\ref{fig:l1l2}---the no-dividend CRR engine produces
near-universally negative Layer-1 edges on AAPL puts---and would need
to be relaxed for cleanly dividend-adjusted underlyings.

\textbf{Layer-2 feature set is intentionally minimal.} XGBoost was
trained on a small, deliberately-selected feature set
(moneyness, DTE, RV-IV spread, volume-to-OI ratio, dollar bid-ask
spread). Richer feature engineering---order-book microstructure
features, intraday volatility surfaces, term-structure features, or
sentiment proxies---would almost certainly improve the Layer-2 Brier
score below $0.211$. The minimality is a deliberate choice to keep
the cross-validation diagnostic interpretable: every feature is
either a contract characteristic or a measurable market quantity that
a retail trader can compute. Feature-set extensions are identified as
future work.

\textbf{Walk-forward backtest assumes frictionless execution beyond
the modeled costs.} The Layer-2 microstructure cost model accounts
for half-spread on entry and exit, Fidelity's per-contract fee
(\$0.65), and a volume-scaled slippage term. It does not model
queue position, partial fills on illiquid contracts, exchange-side
holds, or assignment risk on short positions held through expiration.
These omissions are immaterial for the in-sample diagnostic but would
need calibration for a deployed strategy.

\textbf{Regime classifier is a binary discretization.} The IV/RV-30
classifier labels each day as either herding or normal using fixed
thresholds. The reality is a continuum, and the
$8.5\%$-of-days herding label aggregates episodes of very different
intensity. A continuous regime score---e.g., the IV/RV-30 ratio
itself, or a more elaborate state-space model---would allow finer
control over the Kelly fraction and may reveal additional structure
in the correlation surface.
